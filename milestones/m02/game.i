...
void drawBugs();
# 4 "game.c" 2
# 1 "spritesheetNet.h" 1
# 21 "spritesheetNet.h"
extern const unsigned short spritesheetNetTiles[16384];


extern const unsigned short spritesheetNetPal[256];
# 5 "game.c" 2
...
void initGame() {
    ...
    DMANow(3, spritesheetNetTiles, &((charblock *)0x6000000)[4], 32768 / 2);
    DMANow(3, spritesheetNetPal, ((unsigned short *)0x5000200), 256);
...
void updatePlayer() {
    if ((!(~(oldButtons)&((1<<5))) && (~buttons & ((1<<5)))) && (player.col > 0)) {
        direction = LEFT;
    ...
    if ((!(~(oldButtons)&((1<<4))) && (~buttons & ((1<<4)))) && ((player.col - 1) < 240)) {
        direction = RIGHT;
        player.col += player.cdel;
    }
    if ((~((*(volatile unsigned short *)0x04000130)) & ((1<<4))) && ((player.col - 1) < 240)) {
        direction = RIGHT;
        player.col += player.cdel;
    }
...
void drawBees() {
    shadowOAM[1].attr0 = bees.row | (0<<14) | (0<<13);
    shadowOAM[1].attr1 = bees.col | (2<<14);
...
void drawRocks() {
    for (int i = 0; i < 3; i++) {
        if (rocks[i].active) {
            shadowOAM[2 + i].attr0 = rocks[i].row | (0<<14) | (0<<13);
            shadowOAM[2 + i].attr1 = rocks[i].col | (0<<14);
