...
extern const signed char rockHit[11808];
# 7 "game.c" 2
...
void initPlayer() {
    ...
    player.screenCol = 0;
    player.worldRow = ((ground + vOff) << 8);
    player.worldCol = (240 / 2) - (player.width / 2);
    player.rdel = 0;
    player.cdel = 2;
    player.width = 32;
    player.height = 32;
...
void updatePlayer() {
    ...
    if ((!(~(oldButtons)&((1<<6))) && (~buttons & ((1<<6)))) && !jumping) {
        player.rdel -= 1500;
        jumping = 1;
    }
    ...
    if ((!(~(oldButtons)&((1<<5))) && (~buttons & ((1<<5)))) && (player.screenCol > 0)) {
        direction = LEFT;
        player.worldCol -= player.cdel;
    }
    if ((~((*(volatile unsigned short *)0x04000130)) & ((1<<5))) && (player.screenCol > 0)) {
        direction = LEFT;
        player.worldCol -= player.cdel;
    }


    if ((!(~(oldButtons)&((1<<4))) && (~buttons & ((1<<4)))) && ((player.screenCol - 1) < 240)) {
        direction = RIGHT;
        player.worldCol += player.cdel;
    }
    if ((~((*(volatile unsigned short *)0x04000130)) & ((1<<4))) && ((player.screenCol - 1) < 240)) {
        direction = RIGHT;
        player.worldCol += player.cdel;
    }


    if ((!(~(oldButtons)&((1<<1))) && (~buttons & ((1<<1)))) && !usingNet && cheat == 0) {
        usingNet = 1;
        player.curFrame = 0;
    }
...
void drawPlayer() {
    if (usingNet == 0) {
        if (cheat == 0) {
            ...
            shadowOAM[0].attr0 = (0xFF & player.screenRow) | (0<<14) | (0<<13);
            shadowOAM[0].attr1 = (0x1FF & player.screenCol) | (2<<14);
            shadowOAM[0].attr2 = ((player.curFrame * 4)*32+((player.aniState * 4) + 16)) | ((0)<<12);
        }
    } else {
        shadowOAM[0].attr0 = player.screenRow | (1<<14) | (0<<13);
        shadowOAM[0].attr1 = (player.screenCol - 16) | (3<<14);
        shadowOAM[0].attr2 = (((player.curFrame * 4) + 12)*32+(player.aniState * 8)) | ((0)<<12);
    }
}
...
void drawBees() {
    shadowOAM[1].attr0 = bees.screenRow | (0<<14) | (0<<13);
    shadowOAM[1].attr1 = bees.screenCol | (2<<14);
    shadowOAM[1].attr2 = (((bees.curFrame * 4) + 1)*32+((bees.aniState * 4) + 8)) | ((0)<<12);
}
...
void drawRocks() {
    for (int i = 0; i < 2; i++) {
        if (rocks[i].active) {
            shadowOAM[2 + i].attr0 = rocks[i].screenRow | (0<<14) | (0<<13);
            shadowOAM[2 + i].attr1 = rocks[i].screenCol | (1<<14);
            shadowOAM[2 + i].attr2 = ((2)*32+(14)) | ((0)<<12);
        }
    }
}
...
void drawBugs() {
    for (int i = 0; i < 2; i++) {
        if (bugs[i].active) {
            shadowOAM[2 + 3 + 1 + i].attr0 = bugs[i].screenRow | (0<<14) | (0<<13);
            shadowOAM[2 + 3 + 1 + i].attr1 = bugs[i].screenCol | (1<<14);
...
void drawHearts() {
    for (int i = 0; i < 3; i++) {
        if (hearts[i].active) {
            shadowOAM[2 + 2 + i].attr0 = hearts[i].screenRow | (0<<14) | (0<<13);
            shadowOAM[2 + 2 + i].attr1 = hearts[i].screenCol | (1<<14);
            shadowOAM[2 + 2 + i].attr2 = ((hearts[i].curFrame * 2)*32+((hearts[i].aniState * 2) + 12)) | ((0)<<12);
        } else {
            shadowOAM[2 + 2 + i].attr0 = (2<<8);
        }
    }
}
